...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao'], $_POST['solic_id'])) {
    ...
    // Aprovar transferência — lado origem
    if ($acao === 'aprovar_origem' && $solic['tipo'] === 'transferencia' && $solic['status'] === 'pendente') {
        $db->prepare("UPDATE solicitacoes SET status='aprovado_origem', obs_resposta=?, aprovado_por_origem=? WHERE id=?")
           ->execute([$obs, $u['id'], $sid]);
        $msg = 'Aprovação registrada. Aguardando gerente da loja cedente.';
        goto fim;
    }
...
// ---- LISTAGEM ----
// Gerente vê solicitações da própria loja (como solicitante OU como cedente)
$where = $loja_filtro
...
<style>
    ...
    .btn-sm  { padding:4px 12px; border-radius:5px; border:none; cursor:pointer; font-size:.78rem; font-weight:600; font-family:'Rajdhani',sans-serif; }
        ...
        <table class="data-table">
            ...
            <?php foreach ($solics as $s):
                ...
                $pode_aprovar_orig = $s['status'] === 'pendente'
                    && (!$loja_filtro || $loja_filtro == $s['loja_solicitante']);
                $pode_aprovar_ced  = $s['status'] === 'aprovado_origem'
                    && $s['tipo'] === 'transferencia'
                    && (!$loja_filtro || $loja_filtro == $s['loja_cedente']);
                $pode_rejeitar     = in_array($s['status'], ['pendente','aprovado_origem'])
                    && (!$loja_filtro || $loja_filtro == $s['loja_solicitante'] || $loja_filtro == $s['loja_cedente']);
            ?>
            <tr>
                ...
                <td style="white-space:nowrap">
                    <?php if ($pode_aprovar_orig): ?>
                        <?php $btn_acao = $s['tipo'] === 'transferencia' ? 'aprovar_origem' : 'aprovar'; ?>
                        <button class="btn-sm btn-ok" onclick="abrirModal(<?= $s['id'] ?>,'<?= $btn_acao ?>')">
                            <?= $s['tipo'] === 'transferencia' ? 'Aprovar origem' : 'Aprovar' ?>
                        </button>
                    <?php endif; ?>
